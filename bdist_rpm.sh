...
sudo yum install -y nvidia-driver-devel cuda-nvml-dev-10-2 nvidia-driver-NVML ucx-devel pmix-devel-3.* numactl-devel hwloc-devel
...
#rpmbuild --define "gittag ${GITTAG}" --define "_topdir $PWD" --with numa --with pmix --with hwloc --with x11 --with ucx -ba SPECS/slurm.spec --define "_configure ./configure CPATH=${nvmls[0]}" --define "_smp_mflags CPATH=${nvmls[0]}"
rpmbuild --define "gittag ${GITTAG}" --define "_topdir $PWD" --with pmix --with numa --with hwloc --with mysql --with x11 --with ucx -ba SPECS/$SPEC
